...
class DirectionalReadingOrderPE(nn.Module):
    ...
    def _build_sinusoidal_pe_1d(self, dim, max_size):
        position = torch.arange(max_size, dtype=torch.float32).unsqueeze(1)  # [L, 1]
        pe = torch.zeros(max_size, dim, dtype=torch.float32)  # [L, C]

        even_dim = (dim + 1) // 2
        div_term = torch.exp(
            torch.arange(0, even_dim, dtype=torch.float32) *
            (-np.log(self.scale_base) / float(dim)))
    ...
    def _resolve_direction(self, h, w, direction=None):
        # Extension point: use direction flag from data/ratio-bin later.
    ...
    def forward(self, x, direction=None):
        ...
        if h > self.max_size or w > self.max_size:
            raise ValueError(
                f"feature size exceeds DirectionalPE max_size={self.max_size}: "
                f"H={h}, W={w}. Increase direction_pe_max_size."
            )

        mode = self._resolve_direction(h=h, w=w, direction=direction)
        pe_1d = cast(torch.Tensor, self.pe_1d)
        if mode == "horizontal":
            pe = pe_1d[:, :, :w].unsqueeze(2)  # [1, C, 1, W]
        elif mode == "vertical":
            pe = pe_1d[:, :, :h].unsqueeze(3)  # [1, C, H, 1]
        else:
    ...
    def __init__(self,
        ...
        for i_stage in range(num_stages):
            stage = SVTRStage(
                ...
                direction_pe_max_size=direction_pe_max_size,
            )
