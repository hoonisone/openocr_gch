...
class_to_module['GTCDecoder_Residual_HV'] = (
    'gch.openocr.openrec.modeling.decoder.gtc_decoder_residual_hv'
)


class GTCDecoder_Residual_HV(nn.Module):
    def __init__(
        self,
        in_channels,
        gtc_decoder,
    ...
    def forward(self, x, data=None):
        ...
        if self.rotation_mode == "fusion":
            # fused: [B, C, L, M], L=max(H,W), M=num_slots
            ...
            if self.apply_to_gtc:
                x_gtc = fused.flatten(2).transpose(1, 2).contiguous()
            else:
...
class HVTokenSlotFusion_BCHW(nn.Module):
    ...
    origin branch:
        원본 2D feature를 [B, C, L, M]으로 adaptive pooling.
        대각선/비정형 방향에서 h/v 강제 pooling이 해로울 때 우회 경로 역할.
    ...
    fusion:
        gate over {origin, horizontal, vertical}
        결과: [B, C, L, M]
    """
    ...
    def __init__(
        ...
        # 3-way gate: origin / horizontal / vertical
        # If use_origin_branch=False, this still works as 2-way h/v by using
        # a different gate. For your current experiment, keep origin enabled.
    ...
    def make_origin_slots(
        self,
        x: torch.Tensor,
        target_len: int,
    ) -> torch.Tensor:
        """
        원본 feature를 [B, C, L, M]으로 맞춘다.
        대각선/곡선/비정형 방향에서는 h/v pooling보다 이 branch가 안전한 우회로가 된다.
        ...
        return F.adaptive_avg_pool2d(
            x,
            output_size=(target_len, self.num_slots),
        )
    ...
    def make_horizontal_slots(self, x: torch.Tensor) -> torch.Tensor:
        ...
        patches = x.permute(0, 3, 2, 1).contiguous()  # [B, W, H, C]
        patches = patches.reshape(b * w, h, c)        # [B*W, H, C]

        slots = self.h_resampler(patches)             # [B*W, M, C]

        slots = slots.reshape(b, w, self.num_slots, c)
        slots = slots.permute(0, 3, 1, 2).contiguous() # [B, C, W, M]
    ...
    def make_vertical_slots(self, x: torch.Tensor) -> torch.Tensor:
        ...
        patches = x.permute(0, 2, 3, 1).contiguous()  # [B, H, W, C]
        patches = patches.reshape(b * h, w, c)        # [B*H, W, C]

        slots = self.v_resampler(patches)             # [B*H, M, C]

        slots = slots.reshape(b, h, self.num_slots, c)
        slots = slots.permute(0, 3, 1, 2).contiguous() # [B, C, H, M]
    ...
    def forward(self, x: torch.Tensor, return_aux: bool = False):
        ...
        if self.use_origin_branch:
            orig_aligned = self.make_origin_slots(x, target_len)
        else:
            orig_aligned = None
        ...
        if self.gate_type == "token_slot":
            ...
            if self.use_origin_branch:
                o_g = orig_aligned.permute(0, 2, 3, 1).contiguous()
                gate_input = torch.cat([o_g, h_g, v_g], dim=-1)
                gate_logits = self.gate(gate_input)
                gate = F.softmax(gate_logits, dim=-1)  # [B, L, M, 3]
                gate = gate.permute(0, 3, 1, 2).contiguous()  # [B, 3, L, M]

                fused = (
                    gate[:, 0:1] * orig_aligned
                    + gate[:, 1:2] * h_aligned
                    + gate[:, 2:3] * v_aligned
                )
            else:
                gate_input = torch.cat([h_g, v_g], dim=-1)
                gate_logits = self.gate(gate_input)
            ...
            if self.use_origin_branch:
                o_pool = orig_aligned.mean(dim=3).transpose(1, 2)  # [B, L, C]
                gate_input = torch.cat([o_pool, h_pool, v_pool], dim=-1)
                gate_logits = self.gate(gate_input)
                gate = F.softmax(gate_logits, dim=-1)  # [B, L, 3]
                gate = gate.permute(0, 2, 1).unsqueeze(-1)  # [B, 3, L, 1]
                ...
                gate_input = torch.cat([h_pool, v_pool], dim=-1)
                gate_logits = self.gate(gate_input)
            ...
            # sample-level global gate
            h_pool = h_aligned.mean(dim=(2, 3))  # [B, C]
            v_pool = v_aligned.mean(dim=(2, 3))  # [B, C]

            if self.use_origin_branch:
                o_pool = orig_aligned.mean(dim=(2, 3))  # [B, C]
                gate_input = torch.cat([o_pool, h_pool, v_pool], dim=-1)
                gate_logits = self.gate(gate_input)
                gate = F.softmax(gate_logits, dim=-1)  # [B, 3]

                fused = (
                    gate[:, 0:1, None, None] * orig_aligned
                    + gate[:, 1:2, None, None] * h_aligned
                    + gate[:, 2:3, None, None] * v_aligned
                )
            else:
                gate_input = torch.cat([h_pool, v_pool], dim=-1)
                gate_logits = self.gate(gate_input)
        ...
        if return_aux:
            ...
            if self.use_origin_branch:
                aux["origin_aligned"] = orig_aligned
            return aux
