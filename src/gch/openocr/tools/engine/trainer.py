...
def _map_nested(batch, leaf_transform):
    """Recursively apply leaf_transform to nested batch objects."""
    if isinstance(batch, dict):
        return {k: _map_nested(v, leaf_transform) for k, v in batch.items()}
    if isinstance(batch, list):
        return [_map_nested(v, leaf_transform) for v in batch]
    if isinstance(batch, tuple):
        return tuple(_map_nested(v, leaf_transform) for v in batch)
    return leaf_transform(batch)


def _to_device_leaf(x, device):
