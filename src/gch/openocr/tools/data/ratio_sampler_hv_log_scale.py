...
data_mod.SAMPLER_MODULES["RatioSampler_HV_LogScale"] = (
    "gch.openocr.tools.data.ratio_sampler_hv_log_scale"
)
...
class RatioSampler_HV_LogScale(Sampler):
    ...
    This sampler expects dataset.wh_ratio to be signed bins directly:
        ... -2, -1, 0, 1, 2 ...
    and emits batch tuples as:
        [img_w, img_h, sample_idx, signed_bin]
    """
    ...
    def _shape_from_signed_bin(self, signed_bin):
        ...
        if signed_bin > 0:
            ...
            shape_h = self.base_size
        else:
            shape_w = self.base_size
    ...
    def create_batch(self):
        batch_list = []
        group_to_ids = {}
        for ori_idx, signed_bin in zip(self.indices_rank_i_ori, self.indices_rank_i_ratio):
            key = int(signed_bin)
            if key not in group_to_ids:
                group_to_ids[key] = []
            group_to_ids[key].append(int(ori_idx))

        for signed_bin in sorted(group_to_ids.keys()):
            ratio_ids = np.array(group_to_ids[signed_bin], dtype=np.int64)

            if ratio_ids.shape[0] == 0:
                continue

            if self.shuffle:
                ratio_ids = np.array(random.sample(ratio_ids.tolist(), len(ratio_ids)))

            num_ratio = ratio_ids.shape[0]
            shape_w, shape_h = self._shape_from_signed_bin(signed_bin)
            ...
            if num_ratio > batch_size_ratio:
                ...
                ra_wh = np.full_like(ratio_ids_full, signed_bin)
                ...
                if batch_num_ratio * batch_size_ratio < num_ratio:
                    ...
                    ra_wh = np.full_like(drop, signed_bin)
                ...
                ra_wh = np.full_like(ratio_ids, signed_bin)
