...
def main():
    parser = argparse.ArgumentParser(
        description="Copy PPOCR dataset and rotate vertical images counterclockwise using multiprocessing."
    )
    parser.add_argument(
        "--data_dir",
        required=True,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.5,
        help="Rotate if height / width > threshold. Default: 1.5",
    )
    parser.add_argument(
        "--delimiter",
        default="\t",
    ...
    parser.add_argument(
        "--rotate_equal",
        action="store_true",
        help="Use height / width >= threshold instead of > threshold.",
    )
    parser.add_argument(
        "--allow_json_list",
        action="store_true",
